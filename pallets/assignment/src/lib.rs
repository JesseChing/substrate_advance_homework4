...
pub mod pallet {
	...
	use sp_runtime::{
		...
		RuntimeDebug, biguint::split, Permill,
	};
	...
	const PRICE_REQUEST_URL:&str  = "https://api.coincap.io/v2/assets/polkadot";
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(1000)]
		pub fn submit_price_unsigned(origin: OriginFor<T>,price:(u64, Permill)) -> DispatchResult {
			let _ = ensure_none(origin)?;
		...
		#[pallet::weight(1000)]
		pub fn submit_price_unsigned_with_signed_payload(origin: OriginFor<T>, payload: PricePayload<T::Public>, _signature: T::Signature) -> DispatchResult {
			let _ = ensure_none(origin)?;
	...
	impl<T: Config> Pallet<T> {
		...
		fn add_price(who: T::AccountId, price: (u64, Permill)){
			log::info!("Adding new price: {:?}", price);
			...
			log::info!("The price had been added: {:?}", price);
		...
		fn fetch_price_info() -> Result<(), Error<T>> {
			...
			/**
			 * 每隔一定周期都会查询价格，同时将价格数据上链，且数据上链频繁，可能造成的gas费用过高，
			 * 所以采用交易参数签名的无签名交易
			 **/
			Self::fetch_price_and_send_signed_playload();
		...
		fn fetch_price_and_send_signed() -> Result<(), &'static str> {
			...
			let price = Self::fetch_parse_prices_data().map_err(|_| "Failed to fetch price")?;
		...
		fn fetch_price_and_send_unsigned() -> Result<(), &'static str>{
			log::info!("fetch_price_and_send_unsigned");
			let price = Self::fetch_parse_prices_data().map_err(|_| "Failed to fetch price")?;
		...
		fn fetch_price_and_send_signed_playload() -> Result<(), Error<T>>{
			log::info!("fetch_price_and_send_signed_playload");
			let price = Self::fetch_parse_prices_data().unwrap_or((0,Permill::from_parts(0)));
			let signer = Signer::<T, T::AuthorityId>::any_account();
			if let Some((_, res)) = signer.send_unsigned_transaction(
				|acct| PricePayload { price, public: acct.public.clone() },
				Call::submit_price_unsigned_with_signed_payload
			){
				return res.map_err(|_|{
					log::error!("Failed in submit_price_unsigned_with_signed_payload");
		...
		fn fetch_parse_prices_data() -> Result<(u64, Permill), Error<T>>{
			...
			// Deserializing JSON to struct, thanks to `serde` and `serde_derive`
			let price_info:Value = serde_json::from_str(&resp_str).map_err(|_| <Error<T>>::HttpFetchingError)?;
			let price_str = price_info["data"]["priceUsd"].as_str();
			match price_str {
				Some(v) => {
					log::info!("priceUsd: {}", v);
					let split_str:Vec<&str> = v.split(".").collect();
					log::info!("split_str:{:?}", split_str);
					if split_str.len() < 2 {
						return Err(<Error<T>>::HttpFetchingError);
					}
					let price1 = split_str[0].parse::<u64>().map_err(|_| <Error<T>>::HttpFetchingError)?;
					let price2 = Permill::from_parts(split_str[1].parse::<u64>().map_err(|_| <Error<T>>::HttpFetchingError)? as u32);
					log::info!("小数:{:?}" , price2);
			        let final_fomat = (price1,price2);
			        Ok(final_fomat)
				},
				None => Err(<Error<T>>::HttpFetchingError),
			}
		...
		fn fetch_price_from_remote() -> Result<Vec<u8>, Error<T>> {
			log::info!("sending request to: {}", PRICE_REQUEST_URL);

			// Initiate an external HTTP GET request. This is using high-level wrappers from `sp_runtime`.
			let request = rt_offchain::http::Request::get(PRICE_REQUEST_URL);
